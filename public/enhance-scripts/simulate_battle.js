...
(function () {
    ...
    const LINEUP_REFRESH_POLL_MS = 200;
    const LINEUP_REFRESH_TIMEOUT_MS = 3500;
    ...
    const state = {
        ...
        replayBusy: false,
        playbackPanelSuppressed: false,
        jobProgressView: null,
        jobProgressRaf: 0,
        gameButton: null,
        seedRecordView: {
            sourceId: '',
    ...
    let _gameWindowCache = null;

    const GameEnv = {
        getGameWindow(forceScan = false) {
            if (!forceScan && _gameWindowCache && typeof _gameWindowCache.__require === 'function') {
                return _gameWindowCache;
            }
            if (typeof window.__require === 'function') {
                _gameWindowCache = window;
                return _gameWindowCache;
            }
            try {
                const frames = document.querySelectorAll('iframe');
                for (let i = 0; i < frames.length; i += 1) {
                    const cw = frames[i].contentWindow;
                    if (cw && typeof cw.__require === 'function') {
                        _gameWindowCache = cw;
                        return _gameWindowCache;
                    }
                }
            ...
            _gameWindowCache = null;
            return null;
        },
        getGameWs() {
            const gw = this.getGameWindow();
            if (!gw) {
                return null;
            }
            const ws = gw.ws || (gw.h5websocket && gw.h5websocket.ws) || window.ws || null;
            return this.ensureWsSendAsync(ws);
        },
        ensureWsSendAsync(ws) {
            if (!ws) {
                ws = null;
            }
            if (ws && typeof ws.sendAsync === 'function') {
                return ws;
            }
            const gw = this.getGameWindow();
            const candidates = [
                ws,
                gw && gw.ws,
                gw && gw.h5websocket && gw.h5websocket.ws,
                gw && gw.gameWS,
                window.ws,
                window.gameWS,
            ];
            for (let i = 0; i < candidates.length; i += 1) {
                ...
                if (item && typeof item.sendAsync === 'function') {
                    if (gw && !gw.ws) {
                        try { gw.ws = item; } catch (_) {}
                    }
                    if (!window.ws) {
                        try { window.ws = item; } catch (_) {}
                    }
                    return item;
                }
            }
            try {
                const req = (gw && gw.__require) || window.__require;
                if (typeof req === 'function') {
                    ['H5WebSocket', 'WebSocketManager', 'NetworkManager'].forEach((name) => {
                        if (ws) return;
                        const mod = Utils.safeCall(() => req(name), null);
                        if (!mod) return;
                        const cand = mod.ws || mod.instance || mod.default || mod;
                        if (cand && typeof cand.sendAsync === 'function') {
                            ws = cand;
                        }
                    });
                }
            } catch (_) {}
            return ws && typeof ws.sendAsync === 'function' ? ws : null;
        },
        readRoleIdFromGame() {
            try {
                const gw = this.getGameWindow();
                ...
                const candidates = [];
                if (gw.globalThis && gw.globalThis.ROLE && gw.globalThis.ROLE.roleId) {
                    candidates.push(gw.globalThis.ROLE.roleId);
                }
                if (gw.ROLE && gw.ROLE.roleId) {
                    candidates.push(gw.ROLE.roleId);
                }
                if (gw.ROLE && gw.ROLE.id) {
                    candidates.push(gw.ROLE.id);
                }
                try {
                ...
                try {
                    if (window.__h5web_guestMeta && window.__h5web_guestMeta.roleId) {
                        candidates.push(window.__h5web_guestMeta.roleId);
                    }
                } catch (_) {}
                try {
                    const di = gw.__require && gw.__require('data-index');
                    if (di) {
                        if (di.ROLE && di.ROLE.roleId) candidates.push(di.ROLE.roleId);
                        if (di.ROLE && di.ROLE.id) candidates.push(di.ROLE.id);
                        if (di.role && di.role.roleId) candidates.push(di.role.roleId);
                        if (di.RoleService && di.RoleService.roleId) candidates.push(di.RoleService.roleId);
                    }
                } catch (error) {
                    // ignore
                }
                try {
                    const mm = gw.__require && gw.__require('ModuleManager');
                    const Configs = gw.__require && gw.__require('Configs');
                    if (mm && mm.GET_MODULE && Configs && Configs.ModuleType) {
                        const roleMod = mm.GET_MODULE(Configs.ModuleType.ROLE) || mm.GET_MODULE(Configs.ModuleType.USER);
                        if (roleMod && roleMod.roleId) candidates.push(roleMod.roleId);
                        if (roleMod && roleMod.role && roleMod.role.roleId) candidates.push(roleMod.role.roleId);
                    }
                } catch (error) {
                    // ignore
                }
        ...
        isGameSessionReady() {
            const gw = this.getGameWindow();
            if (!gw || typeof gw.__require !== 'function') {
                return false;
            }
            if (!this.readRoleIdFromGame()) {
                return false;
            }
            const ws = this.getGameWs();
            if (!ws || typeof ws.sendAsync !== 'function') {
                return false;
            }
            return !!document.body;
        },
        waitForGameReady(maxWaitMs = 600000) {
            return new Promise((resolve) => {
                if (this.isGameSessionReady()) {
                    resolve(true);
                    return;
                }
                let n = 0;
                ...
                const poll = () => {
                    ...
                    if (!this.getGameWindow(true) && (Date.now() - t0) > 60000) {
                        resolve(false);
                        return;
                    }
                    ...
                    n += 1;
                    setTimeout(poll, Math.min(5000, 1000 + Math.floor(n / 5) * 1000));
                };
                poll();
            });
        },
    ...
    const Utils = {
        ...
        /**
         * 同步一次角色数据（等价于游戏内"火把刷新" + 换阵后主线重建）：
         * 1) data-index.RoleService.getRoleInfo（与 game.js _trySyncRoleData 同源）
         * 2) LEVEL_BATTLE.resetLevelBattle()（与换阵后 game.js:99212 / 159457 同源）
         * 3) 轮询捕获最新主线 battleData 骨架，避免沿用旧阵容 leftTeam
         *
         * 设计要点：
         * - 1.5 秒内的并发调用合并成同一个 Promise，避免连点"开始模拟"打风暴
         * - 任何失败（ws 未连 / 超时 / code≠0）只 warn 不抛，模拟流程不会被阻塞
         *   （等价于火把刷新失败时游戏也不会卡住，最多用旧数据继续模拟）
         * - 返回 Promise<boolean>：true=已同步，false=失败/不可用，调用方一般忽略
         *
         * @param {string} reason 调用来源（仅日志/调试用），不影响行为
         */
        syncRoleInfo(reason = '') {
            const now = Date.now();
            if (this._lastRoleSyncPromise && (now - (this._lastRoleSyncAt || 0)) < 1500) {
                return this._lastRoleSyncPromise;
            }
            this._lastRoleSyncAt = now;
            const exec = (async () => {
                try {
                    const gw = GameEnv.getGameWindow();
                    if (!gw) {
                        this.warn(`syncRoleInfo 跳过 (${reason}): 游戏窗口未就绪`);
                        return false;
                    }
                    const dataIndex = Utils.safeCall(() => gw.__require('data-index'), null);
                    if (dataIndex && dataIndex.RoleService && typeof dataIndex.RoleService.getRoleInfo === 'function') {
                        const pmMod = Utils.safeCall(() => gw.__require('PlatformManager'), null);
                        const pm = pmMod && pmMod.PlatformManager && pmMod.PlatformManager.instance;
                        const clientVersion = typeof gw.GAME_VERSION !== 'undefined'
                            ? gw.GAME_VERSION
                            : (typeof GAME_VERSION !== 'undefined' ? GAME_VERSION : '');
                        const resp = await dataIndex.RoleService.getRoleInfo({
                            platform: pm ? pm.platformType : '',
                            platformExt: pm ? pm.platformExt : '',
                            inviteUid: 0,
                            clientVersion,
                            scene: '',
                        });
                        if (resp && resp.code === 0) {
                            Utils.safeCall(() => {
                                const gsMod = gw.__require('GlobalSignal');
                                if (gsMod && gsMod.GlobalSignal && gsMod.GlobalSignal.EventRandomSeedUpdate) {
                                    gsMod.GlobalSignal.EventRandomSeedUpdate.dispatch();
                                }
                            }, null);
                            this.log(`syncRoleInfo 已同步 (${reason}) [RoleService]`);
                            await refreshMainLineBattleAfterLineupSync(reason);
                            return true;
                        }
                        this.warn(`syncRoleInfo RoleService 返回非 0 (${reason}): code=${resp && resp.code}`);
                        return false;
                    }
                    const ws = GameEnv.getGameWs();
                    if (!ws || typeof ws.sendAsync !== 'function') {
                        this.warn(`syncRoleInfo 跳过 (${reason}): WebSocket sendAsync 不可用`);
                        return false;
                    }
                    const resp = await ws.sendAsync({
                        ack: 0,
                    ...
                    if (resp && resp.code === 0) {
                        this.log(`syncRoleInfo 已同步 (${reason}) [ws]`);
                        await refreshMainLineBattleAfterLineupSync(reason);
                        return true;
                    }
                    this.warn(`syncRoleInfo 服务端返回非 0 (${reason}): code=${resp && resp.code}`);
                    return false;
                } catch (err) {
                    this.warn(`syncRoleInfo 失败 (${reason}):`, err && err.message);
                    return false;
                }
            })();
            this._lastRoleSyncPromise = exec;
            return exec;
        },
    ...
    const Runtime = {
        ...
        require(moduleName) {
            ...
            const gw = GameEnv.getGameWindow();
            const req = (gw && gw.__require) || window.__require || window.require;
            if (typeof req !== 'function') {
        ...
        isReady() {
            return GameEnv.isGameSessionReady();
        },
        getConfigs() {
    ...
    function getLevelBattleSkeletonSignature() {
        ...
        return battleData ? getCurrentTeamSignatureFromBattleData(battleData) : '';
    }

    /**
     * 换阵/同步角色后重建主线战斗并刷新可复用骨架（对齐游戏切换阵容后的 resetLevelBattle 流程）。
     */
    async function refreshMainLineBattleAfterLineupSync(reason = '') {
        const beforeSig = getLevelBattleSkeletonSignature()
            || getCurrentTeamSignatureFromBattleData(state.reusableSkeleton);
        let triggered = false;
        const levelModule = Utils.safeCall(() => Runtime.getModuleByTypeKey('LEVEL_BATTLE'), null);
        if (levelModule && typeof levelModule.resetLevelBattle === 'function') {
            Utils.safeCall(() => levelModule.resetLevelBattle(), null);
            triggered = true;
            Utils.pushDataSource(`换阵同步：LEVEL_BATTLE.resetLevelBattle (${reason})`);
        } else {
            const GlobalSignal = Utils.safeCall(() => Runtime.getGlobalSignal(), null);
            if (GlobalSignal && GlobalSignal.LevelBattleReset && typeof GlobalSignal.LevelBattleReset.emit === 'function') {
                Utils.safeCall(() => GlobalSignal.LevelBattleReset.emit(), null);
                triggered = true;
                Utils.pushDataSource(`换阵同步：LevelBattleReset.emit (${reason})`);
            }
        }
        if (!triggered) {
            Utils.warn(`换阵同步：无法触发主线重置 (${reason})，尝试直接捕获`);
            const captured = tryCaptureAutoLevelBattleSkeleton(`syncLineup.fallback.${reason}`);
            if (captured) {
                state.activeSwitchedCandidate = null;
            }
            return captured;
        }
        const deadline = Utils.now() + LINEUP_REFRESH_TIMEOUT_MS;
        while (Utils.now() < deadline) {
            await Utils.sleep(LINEUP_REFRESH_POLL_MS);
            if (!tryCaptureAutoLevelBattleSkeleton(`syncLineup.${reason}`)) {
                continue;
            }
            const afterSig = getCurrentTeamSignatureFromBattleData(state.reusableSkeleton);
            if (afterSig && afterSig !== beforeSig) {
                Utils.pushDataSource(`换阵同步：主线骨架已更新 (${reason})`);
                state.activeSwitchedCandidate = null;
                return true;
            }
        ...
        const captured = tryCaptureAutoLevelBattleSkeleton(`syncLineup.timeout.${reason}`);
        if (captured) {
            state.activeSwitchedCandidate = null;
            Utils.pushDataSource(`换阵同步：超时后仍捕获到主线骨架 (${reason})`);
        } else {
            Utils.warn(`换阵同步：主线骨架刷新超时 (${reason})`);
        }
        return captured;
    }
    ...
    function restorePlaybackBattleVisible(source = 'playback') {
        ...
        restorePanelAfterPlayback(source);
    }

    function hidePanelForPlayback(source = 'playback') {
        if (state.playbackPanelSuppressed || !state.panel) {
            return;
        }
        state.playbackPanelSuppressed = true;
        state.panelPrefs.collapsed = true;
        renderPanel();
        Utils.pushDataSource(`hide sim panel (${source})`);
    }

    function restorePanelAfterPlayback(source = 'playback') {
        if (!state.playbackPanelSuppressed) {
            return;
        }
        state.playbackPanelSuppressed = false;
        state.panelPrefs.collapsed = false;
        savePanelState();
        renderPanel();
        Utils.pushDataSource(`restore sim panel (${source})`);
    }
    ...
    function findKnownTeamEntryByHeroId(heroId, preferredBattleData = null) {
        ...
        const levelBattle = Utils.safeCall(() => Runtime.getLevelBattle(), null);
        const liveMainlineData = Utils.safeCall(() => levelBattle && levelBattle.options ? levelBattle.options.battleData : null, null);
    ...
    function captureCurrentTeamSnapshot(limitations = null) {
        ...
        const candidate = {
            ...
            leftTeamMeta: Utils.toSerializable((() => {
                const proxyMeta = {};
                if (reusable && reusable.leftTeam) {
                    Object.keys(reusable.leftTeam).forEach((key) => {
                        if (key !== 'team' && key !== 'id') {
                            proxyMeta[key] = reusable.leftTeam[key];
                        }
                    });
                }
                proxyMeta.headImg = proxyMeta.headImg || roleMeta.headImg;
                proxyMeta.name = proxyMeta.name || roleMeta.name;
                proxyMeta.roleId = proxyMeta.roleId || roleMeta.roleId;
                proxyMeta.weaponId = liveWeaponId;
                return proxyMeta;
            })()),
    ...
    function applyCandidateToBattleData(battleData, candidate) {
        ...
        Object.keys(leftTeamMeta).forEach((key) => {
            if (key !== 'team' && key !== 'id' && leftTeamMeta[key] !== undefined) {
    ...
    function finalizeBattleDataForSimulation(battleData) {
        ...
        if (battleData.leftTeam && battleData.rightTeam) {
            const syncMissingIterationProps = (source, target) => {
            ...
            syncMissingIterationProps(battleData.leftTeam, battleData.rightTeam);
            let sampleLeftEntry = null;
            if (battleData.leftTeam.team) {
                const leftEntries = normalizeTeamEntries(battleData.leftTeam.team);
                if (leftEntries.length > 0 && leftEntries[0].entry) {
                    sampleLeftEntry = leftEntries[0].entry;
                }
            }
            if (sampleLeftEntry && battleData.rightTeam.team) {
                normalizeTeamEntries(battleData.rightTeam.team).forEach(({ entry }) => {
                    if (entry && typeof entry === 'object') {
                        syncMissingIterationProps(sampleLeftEntry, entry);
                    }
                });
            }
        }
        ...
        const entries = normalizeMonsterTriples(monsters).map((item) => ({
            ...
            entry: (() => {
                const entry = Object.assign(
                    buildEmptyEnemyEntry(item.monsterId, item.index, item.level),
    ...
    async function evaluateCandidate(adapter, draft, candidate, seeds, progressLabel) {
        ...
        for (let i = 0; i < seeds.length; i += 1) {
            ...
            if (state.panel) {
                const progressEvery = seeds.length <= 30 ? 1 : (seeds.length <= 200 ? 2 : 5);
                if (i === 0 || (i + 1) % progressEvery === 0 || i === seeds.length - 1) {
                    scheduleJobProgressRender(
                        `${progressLabel} ${formatSignatureShort(candidate.signature)}`,
                        `进度 ${i + 1}/${seeds.length}，当前胜率 ${Utils.formatPercent(aggregate.winCount / aggregate.finishedCount)}`,
                    );
                }
            }
        }
    ...
    async function debugCurrentTeamOnce() {
        return withJob('单次调试模拟', async () => {
            await Utils.syncRoleInfo('debugCurrentTeamOnce');
    ...
    function renderJobRuntimeView() {
        if (!state.panel || !state.jobProgressView) {
            return;
        }
        const refs = state.panel.refs;
        if (state.jobProgressView.autoLogTab) {
            switchPanelToLogTab();
            state.jobProgressView.autoLogTab = false;
        }
        if (refs.job && state.activeJob) {
            refs.job.textContent = `${state.activeJob.title} / 运行中`;
        }
        if (refs.reportBody && state.jobProgressView.text != null) {
            renderLastReport({
                title: state.jobProgressView.title || (state.activeJob && state.activeJob.title) || '任务进度',
                text: state.jobProgressView.text,
            }, state.jobProgressView.reports || null);
        }
    }

    function scheduleJobProgressRender(title, text, options = {}) {
        const autoLogTab = options.autoLogTab != null
            ? !!options.autoLogTab
            : !(state.jobProgressView && state.jobProgressView.text);
        state.jobProgressView = {
            title,
            text,
            reports: options.reports || null,
            autoLogTab,
        };
        if (state.jobProgressRaf) {
            return;
        }
        state.jobProgressRaf = requestAnimationFrame(() => {
            state.jobProgressRaf = 0;
            renderJobRuntimeView();
        });
    }

    function clearJobProgressView() {
        state.jobProgressView = null;
        if (state.jobProgressRaf) {
            cancelAnimationFrame(state.jobProgressRaf);
            state.jobProgressRaf = 0;
        }
    }
    ...
    async function startMainPush() {
        ...
        await Utils.syncRoleInfo('startMainPush');
        const GlobalSignal = Utils.safeCall(() => Runtime.getGlobalSignal(), null);
    ...
    async function withJob(title, runner) {
        ...
        scheduleJobProgressRender(title, '任务启动中，请稍候...', { autoLogTab: true });
        renderPanel();
        try {
            return await runner();
        } finally {
    ...
    async function evaluateCurrentTeam(sampleCount, title) {
        return withJob(title, async () => {
            await Utils.syncRoleInfo(title || 'evaluateCurrentTeam');
            const adapter = getActiveAdapter();
            if (!adapter) {
                throw new Error('未识别到当前可模拟的非PVP玩法');
            }
            ...
            scheduleJobProgressRender(title, `准备模拟，共 ${seeds.length} 次...`, { autoLogTab: true });
    ...
    async function compareCandidates(sampleCount) {
        return withJob(`批量比较 ${sampleCount} 次`, async () => {
            await Utils.syncRoleInfo('compareCandidates');
            ...
            for (let i = 0; i < state.candidates.length; i += 1) {
                ...
                scheduleJobProgressRender(
                    `批量比较 ${i + 1}/${state.candidates.length}`,
                    `正在评估 ${formatSignatureShort(candidate.signature)}...`,
                );
                const draft = adapter.buildDraft(candidate);
    ...
    async function replayLatest() {
        ...
        try {
            ...
            restorePanelAfterPlayback('replay:error');
            throw error;
        } finally {
    ...
    async function _replayLatestInner() {
        ...
        hidePanelForPlayback(`replay:${modeKey || 'capture'}`);
        if (modeKey === 'nightmare') {
    ...
    function attachSimButtonToMainPanelInstance(panelInstance, gwFgui) {
        if (!panelInstance || panelInstance._simEmbedBtn || !panelInstance.ui) {
            return false;
        }
        try {
            const btn = gwFgui.UIPackage.createObject('ui_common', 'BtnInfo2');
            if (!btn) {
                return false;
            }
            btn.title = '';
            ...
            if (btn.numChildren > 0) {
                for (let i = 0; i < btn.numChildren; i += 1) {
            ...
            const textField = new gwFgui.GTextField();
            ...
            textField.setPosition(
                (btn.width - textField.width) / 2,
                (btn.height - textField.height) / 2
            );
            ...
            const paddingY = 20;
            btn.setPosition(panelInstance.ui.width - btn.width - paddingX, paddingY);
            panelInstance.ui.addChild(btn);
            panelInstance._simEmbedBtn = btn;
            return true;
        } catch (e) {
    ...
    function injectMainPanelButton() {
        if (state.mainPanelHookInstalled) {
            return;
        }
        let checkCount = 0;
        const maxChecks = 180;
        const injectionInterval = setInterval(() => {
            checkCount += 1;
            const gw = GameEnv.getGameWindow();
            const gwFgui = (gw && gw.fgui) || (typeof fgui !== 'undefined' ? fgui : null);
            if (!gw || typeof gw.__require !== 'function' || !gwFgui) {
                if (checkCount >= maxChecks) {
                    clearInterval(injectionInterval);
                    Utils.warn('MainPanel 内嵌按钮：游戏环境未就绪');
                }
                return;
            }
            try {
                const MainPanel = gw.__require('MainPanel');
                if (!MainPanel || !MainPanel.MainPanel) {
                    if (checkCount >= maxChecks) {
                        clearInterval(injectionInterval);
                        Utils.warn('MainPanel 模块未找到');
                    }
                    return;
                }
                if (!state.mainPanelHookInstalled) {
                    ...
                    MainPanel.MainPanel.prototype.onShow = function (...args) {
                        ...
                        attachSimButtonToMainPanelInstance(this, gwFgui);
                    };
                    ...
                    state.mainPanelHookInstalled = true;
                    Utils.log('【模拟】MainPanel.onShow 已 Hook');
                }
                try {
                    const idx = gw.__require('index-ui');
                    const ui = idx && idx.default ? idx.default : idx;
                    const proxy = ui && ui.GET_PROXY && ui.GET_PROXY('MainPanel');
                    if (proxy && attachSimButtonToMainPanelInstance(proxy, gwFgui)) {
                        Utils.log('【模拟】当前 MainPanel 实例已注入按钮');
                    }
                } catch (_) {}
                clearInterval(injectionInterval);
            } catch (error) {
                if (checkCount >= maxChecks) {
                    clearInterval(injectionInterval);
                    Utils.warn('MainPanel 内嵌按钮注入失败', error);
                }
            }
        }, 800);
    }
    ...
    function createPanel() {
        ...
        document.getElementById('non-pvp-sim-style').textContent = `
            ...
            #non-pvp-sim-panel.collapsed .xc-body {
                display: none !important;
            }
            ...
            try {
                ...
                if (action === 'detect') {
                    ...
                    if (state.mainPush.isRunning) {
                        ...
                        // 等价火把刷新：让游戏先拉一次最新角色数据再开模拟推关
                        await Utils.syncRoleInfo('mainPush');
                        await startMainPush();
                        Utils.toast('主线模拟推关已启动', 'success');
                    }
                } else if (action === 'debugOnce') {
                    Utils.toast('开始单次调试...', 'info');
                    // 等价火把刷新：避免用旧装备 / 旧阵容数据调试
                    await Utils.syncRoleInfo('debugOnce');
                    ...
                    // 等价火把刷新：开始单队模拟前同步一次最新角色数据，避免用旧 BattleData
                    await Utils.syncRoleInfo('quick');
                    ...
                    // 等价火把刷新：批量比较多套候选阵容前同步一次最新角色数据
                    await Utils.syncRoleInfo('compare');
    ...
    function removeLegacyLaunchers() {
        const legacy = document.getElementById('non-pvp-sim-launcher');
        if (legacy) {
            legacy.remove();
        }
    }
    ...
    async function bootstrap() {
        ...
        const ready = await GameEnv.waitForGameReady(600000);
        if (!ready) {
            Utils.warn('等待游戏加载/登录超时，3秒后重试注入');
            setTimeout(bootstrap, 3000);
            return;
        }
        Utils.log('游戏会话已就绪，开始注入战斗模拟脚本');
        if (!Runtime.isReady()) {
