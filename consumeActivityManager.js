...
class ConsumeActivityManager {
  ...
  getAllResourceQuantities(roleData) {
    const items = roleData?.items || roleData?.itemList || roleData?.bag?.items || roleData?.inventory || null;
  ...
  getChestPointData(roleData) {
    const items = roleData?.items || roleData?.itemList || roleData?.bag?.items || {};
...
module.exports = ConsumeActivityManager;
