...
/** 云端配置槽位名校验（1-20位中文/字母/数字/下划线/连字符，排除冒号防 KV 键注入）；空或『默认』返回 '' 表示旧版默认槽 */
const CLOUD_SLOT_RE = /^[a-zA-Z0-9_\u4e00-\u9fa5-]{1,20}$/;
function normalizeCloudSlot(name) {
  const n = String(name || '').trim();
  if (!n || n === '默认') return '';
  return n;
}

async function cloudApproved(kv, username) {
  try {
    const uRaw = await kv.get(`clouduser:${username}`);
    if (uRaw) return !!JSON.parse(uRaw).approved;
  } catch { /* ignore */ }
  return false;
}

/** 列出命名槽位元信息（不含旧版默认槽） */
async function listCloudSlots(kv, username) {
  const slots = [];
  let cursor;
  do {
    const page = await kv.list({ prefix: `cloudcfg:${username}:`, cursor });
    for (const k of page.keys) {
      const meta = { name: k.name.slice(`cloudcfg:${username}:`.length), updatedAt: null, size: 0, keys: 0 };
      try {
        ...
        if (raw) {
          ...
          meta.keys = Object.keys(cfg.data || {}).length;
        }
      } catch { /* ignore */ }
      slots.push(meta);
    }
...
async function handleCloudApi(request, env, url, corsHeaders) {
  ...
  if (path === '/api/cloud/register' && request.method === 'POST') {
    ...
    if (await kv.get(`clouduser:${username}`)) return cloudJsonResp({ error: '用户名已存在' }, 400, corsHeaders);
  ...
  if (path === '/api/cloud/login' && request.method === 'POST') {
    ...
    const cfgRaw = await kv.get(`cloudcfg:${username}`);
    if (cfgRaw) {
      try { hasConfig = true; configUpdatedAt = JSON.parse(cfgRaw).updatedAt || null; } catch { /* ignore */ }
    }
    if (!hasConfig) {
      const namedPage = await kv.list({ prefix: `cloudcfg:${username}:`, limit: 1 });
      hasConfig = namedPage.keys.length > 0;
    }
    return cloudJsonResp({ apiToken: user.apiToken, username, hasConfig, configUpdatedAt, approved: !!user.approved }, 200, corsHeaders);
  }
  ...
  // 配置读取/上传/删除（需登录；name 参数指定槽位，缺省为旧版默认槽）
  if (path === '/api/cloud/config') {
    const username = await authCloudUser(env, request);
    if (!username) return cloudJsonResp({ error: '未登录或登录已失效' }, 401, corsHeaders);
    // 授权校验：未授权账号禁止上传/下载
    const approved = await cloudApproved(kv, username);
    if (!approved) return cloudJsonResp({ error: '账号未授权，请联系管理员在云端后台开通后重试' }, 403, corsHeaders);

    const slot = normalizeCloudSlot(url.searchParams.get('name'));
    if (slot && !CLOUD_SLOT_RE.test(slot)) {
      return cloudJsonResp({ error: '配置名称需为1-20位（中文/字母/数字/下划线/连字符）' }, 400, corsHeaders);
    }
    const cfgKey = slot ? `cloudcfg:${username}:${slot}` : `cloudcfg:${username}`;

    if (request.method === 'GET') {
      const raw = await kv.get(cfgKey);
      if (!raw) return cloudJsonResp({ error: '云端暂无该配置' }, 404, corsHeaders);
    ...
    if (request.method === 'PUT') {
      ...
      // 命名槽位数量上限 10（已存在的槽位覆盖不计入）
      if (slot && !(await kv.get(cfgKey))) {
        const existing = await listCloudSlots(kv, username);
        if (existing.length >= 10) {
          return cloudJsonResp({ error: '命名配置最多10个，请先删除旧配置' }, 413, corsHeaders);
        }
      }
      const payload = { name: slot || '默认', updatedAt: new Date().toISOString(), data: body.data };
      const serialized = JSON.stringify(payload);
      if (serialized.length > 4 * 1024 * 1024) {
        return cloudJsonResp({ error: '配置大小超过4MB限制' }, 413, corsHeaders);
      }
      await kv.put(cfgKey, serialized);
      return cloudJsonResp({ updatedAt: payload.updatedAt, name: slot || '默认' }, 200, corsHeaders);
    }

    if (request.method === 'DELETE') {
      await kv.delete(cfgKey);
      return cloudJsonResp({ ok: true }, 200, corsHeaders);
    }
  }

  // 配置列表（需登录）：默认槽 + 命名槽，按更新时间倒序
  if (path === '/api/cloud/configs' && request.method === 'GET') {
    const username = await authCloudUser(env, request);
    if (!username) return cloudJsonResp({ error: '未登录或登录已失效' }, 401, corsHeaders);
    const approved = await cloudApproved(kv, username);
    if (!approved) return cloudJsonResp({ error: '账号未授权，请联系管理员在云端后台开通后重试' }, 403, corsHeaders);
    const configs = await listCloudSlots(kv, username);
    const legacyRaw = await kv.get(`cloudcfg:${username}`);
    if (legacyRaw) {
      const meta = { name: '默认', updatedAt: null, size: legacyRaw.length, keys: 0 };
      try {
  ...
  if (path.startsWith('/api/cloud/admin/')) {
    ...
    if (path === '/api/cloud/admin/users' && request.method === 'GET') {
      ...
      do {
        ...
        for (const k of page.keys) {
          ...
          let configSize = 0;
          let configKeys = 0;
          const cfgRaw = await kv.get(`cloudcfg:${uname}`);
          if (cfgRaw) {
            hasConfig = true;
            configSize = cfgRaw.length;
            try {
              const cfg = JSON.parse(cfgRaw);
              configUpdatedAt = cfg.updatedAt || null;
              configKeys = Object.keys(cfg.data || {}).length;
            } catch { /* ignore */ }
          }
          users.push({
            username: uname,
    ...
    if (path === '/api/cloud/admin/config' && request.method === 'GET') {
      ...
      const raw = await kv.get(`cloudcfg:${uname}`);
    ...
    if (path === '/api/cloud/admin/user' && request.method === 'DELETE') {
      ...
      await kv.delete(`cloudcfg:${uname}`);
      // 清理命名配置槽位
      let slotCursor;
      do {
        const slotPage = await kv.list({ prefix: `cloudcfg:${uname}:`, cursor: slotCursor });
        for (const k of slotPage.keys) await kv.delete(k.name);
        slotCursor = slotPage.list_complete ? undefined : slotPage.cursor;
      } while (slotCursor);
      return cloudJsonResp({ ok: true }, 200, corsHeaders);
    }
  }
