/**
 * 探测：跳过 bin，直接用 code/combUser 查询角色信息的可行端点
 * 1) comb-platform 侧：是否存在直接吃 combUser 的 serverlist/roles 端点
 * 2) 游戏服务器侧：是否存在直接吃 code 或 combUser 的登录端点
 */
const YYB = 'http://127.0.0.1:8000';
const APP_ID = 'wx0840558555a454ed';
const COMB = 'https://comb-platform.hortorgames.com';
const GAME = 'https://xxz-xyzw.hortorgames.com';

// ---- 加密（与 hortorLogin.ts 一致）----
const cipherTable =
  "";
const leftSide = (s) => (s.length % 2 ? null : s.substring(0, s.length / 2));
const rightSide = (s) => (s.length % 2 ? null : s.substring(s.length / 2));
const transCode = (str, times) => {
  if (times <= 0) return str;
  if (str.length % 2) return null;
  const r = transCode(rightSide(str), times - 1);
  const l = transCode(leftSide(str), times - 1);
  return r === null || l === null ? null : r + l;
};
const getCodeKey = (str, step) => {
  const chars = str.split('');
  const out = [];
  for (let i = 0; i < Math.floor(str.length / step); i++) out.push(chars[i * step]);
  return out.join('');
};
const dealWithString = (src, key, shift) => {
  const v = src.split(''), w = key.split(''), out = new Array(v.length);
  let idx = w.length >> shift;
  for (let i = 0; i < v.length; i++) {
    if (idx >= w.length) idx = 0;
    out[i] = String.fromCharCode(v[i].charCodeAt(0) ^ w[idx].charCodeAt(0));
    idx++;
  }
  return out.join('');
};
const b64 = (t) => Buffer.from(t, 'utf8').toString('base64');
const encodePayload = (text) => {
  const mid = dealWithString(b64(text), getCodeKey(transCode(cipherTable, 6), 3), 1);
  return Buffer.from(mid, 'latin1').toString('base64');
};

const brief = async (resp) => {
  const text = await resp.text();
  return `HTTP ${resp.status} | ${text.slice(0, 220).replace(/\s+/g, ' ')}`;
};

(async () => {
  // 1) getCode
  const codeResp = await (await fetch(YYB + '/wxapp/getCode', {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ ref: 'owNAX6r5qZdH6omhKvsN0hJA7jF4', app_id: APP_ID }),
  })).json();
  const code = codeResp?.data?.result?.code;
  if (!code) { console.log('getCode 失败:', JSON.stringify(codeResp)); return; }
  console.log('[0] code =', code);

  // 2) comb-login 拿 combUser
  const payload = {
    gameId: 'xyzwprod', code, gameTp: 'minigame', version: '1.91.1-wx',
    sysInfo: '{"system":"Android","hortorSDKVersion":"4.0.6-cn","model":"22081212C","brand":"Redmi"}',
    channel: 'hortor', appFrom: 'com.tencent.mm', noLogin: '2',
    distinctId: 'DID-a38175b7-14ce-4b36-aa89-3e092ea03ea6', state: 'hortor',
    packageName: 'com.hortor.games.xyzw', tp: 'mini-we',
    signPrint: 'E6:F7:FE:A9:EC:8E:24:D0:4F:2A:32:50:28:78:E1:C5:5E:70:81:13',
  };
  const loginResp = await fetch(COMB + '/comb-login-server/api/v1/login?gameId=xyzwprod&gameTp=minigame&system=android&version=1.91.1-wx&deviceUniqueId=ck42mn8i&loginTag=code&cryptVersion=1.1.0', {
    method: 'POST', headers: { 'Content-Type': 'text/plain; charset=utf-8' },
    body: encodePayload(JSON.stringify(payload)),
  }).then((r) => r.json());
  const combUser = loginResp?.data?.combUser;
  if (!combUser) { console.log('comb-login 失败:', JSON.stringify(loginResp)); return; }
  console.log('[1] combUser =', JSON.stringify(combUser));
  const sdkInfo = loginResp?.data?.combSdkInfo || {};

  const H = { 'Content-Type': 'application/json', Accept: '*/*' };

  // 3) comb-platform 侧候选端点（直接吃 combUser/code 查角色/服务器）
  const combProbes = [
    ['POST', COMB + '/comb-login-server/api/v1/serverlist', { gameId: 'xyzwprod', combUser }],
    ['POST', COMB + '/comb-login-server/api/v1/serverList', { gameId: 'xyzwprod', combUser }],
    ['POST', COMB + '/comb-login-server/api/v1/roles', { gameId: 'xyzwprod', combUser }],
    ['POST', COMB + '/comb-login-server/api/v1/userinfo', { gameId: 'xyzwprod', combUser }],
    ['POST', COMB + '/comb-login-server/api/v1/auth', { gameId: 'xyzwprod', combUser }],
    ['POST', COMB + '/comb-game-server/api/v1/serverlist', { gameId: 'xyzwprod', combUser }],
    ['POST', COMB + '/comb-platform-server/api/v1/serverlist', { gameId: 'xyzwprod', combUser }],
    ['GET', COMB + '/comb-login-server/api/v1/serverlist?gameId=xyzwprod&userId=' + encodeURIComponent(sdkInfo.userId || ''), null],
  ];
  console.log('\n[2] comb-platform 侧探测:');
  for (const [method, url, body] of combProbes) {
    try {
      const resp = await fetch(url, { method, headers: H, body: body ? JSON.stringify(body) : undefined });
      console.log(`  ${method} ${url.replace(COMB, '')} -> ${await brief(resp)}`);
    } catch (e) {
      console.log(`  ${method} ${url.replace(COMB, '')} -> ERR ${e.message}`);
    }
  }

  // 4) 游戏服务器侧：不走 bin，直接 JSON 提交 combUser / code
  const gameProbes = [
    ['POST', GAME + '/login/serverlist', JSON.stringify({ platform: 'hortor', platformExt: 'mix', info: combUser }), { 'Content-Type': 'application/json' }],
    ['POST', GAME + '/login/authuser', JSON.stringify({ platform: 'hortor', platformExt: 'mix', info: combUser }), { 'Content-Type': 'application/json' }],
    ['POST', GAME + '/login/code', JSON.stringify({ gameId: 'xyzwprod', code }), { 'Content-Type': 'application/json' }],
    ['GET', GAME + '/login/serverlist?code=' + encodeURIComponent(code), null, {}],
  ];
  console.log('\n[3] 游戏服务器侧探测（绕过 bin 直接提交）:');
  for (const [method, url, body, headers] of gameProbes) {
    try {
      const resp = await fetch(url, { method, headers, body: body || undefined });
      console.log(`  ${method} ${url.replace(GAME, '')} -> ${await brief(resp)}`);
    } catch (e) {
      console.log(`  ${method} ${url.replace(GAME, '')} -> ERR ${e.message}`);
    }
  }
})().catch((e) => console.error('ERR', e));
