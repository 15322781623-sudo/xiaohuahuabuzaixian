      ...
      <div v-if="tokenStore.hasTokens" class="tokens-section">
        ...
        <div class="section-header">
          <div class="header-row header-row-title">
            <div class="header-title-group">
              <h2>我的Token列表</h2>
              <n-tag size="small" type="info" round>{{ tokenStore.gameTokens.length }}个</n-tag>
            </div>
            <div class="header-actions">
              <n-button type="success" @click="goToDashboard" size="small">
                <template #icon>
                  <NIcon><List></List></NIcon>
                </template>
                批量功能
              </n-button>
              <n-button
              ...
              <n-dropdown :options="bulkOptions" @select="handleBulkAction" trigger="click">
                <n-button size="small">
                  <template #icon>
          ...
          <div class="header-row header-row-controls">
            <div class="controls-left">
              ...
              <n-divider vertical style="height: 20px; margin: 0 8px;"></n-divider>
              <n-checkbox
              ...
              >
                全选
                <span v-if="selectedTokenIds.size > 0" class="selected-count">
        ...
        <div v-if="viewMode === 'card'" class="tokens-grid">
          <a-card
            ...
            :class="{
              ...
              'multi-selected': selectedTokenIds.has(token.id),
            }"
            ...
            <template #title>
              <a-space align="center" class="token-name">
                <n-checkbox
                  ...
                  @click.stop
                  @update:checked="(checked) => handleTokenSelect(token.id, checked)"
                ></n-checkbox>
                <n-avatar
                  v-if="token.avatar"
                  round
                ...
                <n-tag
                  ...
                  :type="getAutoRefreshStatusType(token.id)"
                >
                  {{ getAutoRefreshStatusText(token.id) }}
                </n-tag>
                <!-- 连接状态文字 -->
                <!-- <a-tag color="green">
        ...
        <div v-else class="tokens-list">
          <div
            ...
            :class="{ active: selectedTokenId === token.id, 'multi-selected': selectedTokenIds.has(token.id) }"
            draggable="true"
            @click="selectToken(token)"
            @dragover="handleDragOver($event)"
            @dragstart="handleDragStart(index, $event)"
          ...
          >
            ...
            <div class="item-main">
              <div class="item-info-row">
                ...
                <div class="item-text">
                  ...
                  <div class="item-remark-row" @click.stop="startEditRemark(token)" v-if="editingRemark !== token.id">
                    <span class="remark-text">{{ token.remark || "添加备注..." }}</span>
                    <NIcon class="remark-icon"><Create></Create></NIcon>
                  </div>
                  <div class="item-remark-edit" @click.stop v-else>
                    <n-input
                      autofocus
                      placeholder="添加备注..."
                      size="tiny"
            ...
            <div class="item-right">
              ...
              <n-button
                ...
                @click.stop="upgradeTokenToPermanent(token)"
              >
                <template #icon><NIcon><Star></Star></NIcon></template>
              </n-button>
              <div class="item-actions">
                ...
                >
                  <template #icon><NIcon><Home></Home></NIcon></template>
                  控制台
                </n-button>
                <n-button
                  size="tiny"
                ...
                <n-dropdown
                  ...
                  trigger="click"
                >
                  <n-button circle size="tiny" @click.stop quaternary>
                    <template #icon><NIcon><EllipsisHorizontal></EllipsisHorizontal></NIcon></template>
                  </n-button>
                </n-dropdown>
              </div>
            </div>
...
// 从 localStorage 读取上次的视图模式，默认为列表视图
...
const isAllSelected = computed(() => {
  return (
    selectedTokenIds.value.size === sortedTokens.value.length &&
...
const handleSelectAll = (checked) => {
  if (checked) {
    selectedTokenIds.value = new Set(sortedTokens.value.map((t) => t.id));
  } else {
...
const bulkOptions = [
  ...
  { type: "divider", key: "d1" },
  { label: "删除选中账号", key: "deleteSelected" },
  { type: "divider", key: "d2" },
...
const refreshToken = async (token) => {
  ...
  // 设置自动刷新状态为待处理
  tokenStore.tokenAutoRefreshStatus[token.id] = {
  ...
  try {
    if (token.importMethod === "url") {
      ...
      // 刷新成功后，无论当前是否有连接，都尝试重新连接以使用新token
      const updatedToken = tokenStore.gameTokens.find((t) => t.id === token.id);
      if (updatedToken) {
        // 如果当前有连接，先关闭
        ...
        setTimeout(() => {
          tokenStore.createWebSocketConnection(
            token.id,
            updatedToken.token, // 使用更新后的token
            token.wsUrl,
          );
        }, 500);
      }
      ...
      // 清除自动刷新状态（手动导入的Token不支持自动刷新）
      delete tokenStore.tokenAutoRefreshStatus[token.id];
...
// 自动刷新状态相关方法
const getAutoRefreshStatus = (tokenId) => {
  return tokenStore.tokenAutoRefreshStatus[tokenId];
};

const getAutoRefreshStatusType = (tokenId) => {
  const status = tokenStore.tokenAutoRefreshStatus[tokenId];
  if (!status)
    return "default";
...
const getAutoRefreshStatusText = (tokenId) => {
  const status = tokenStore.tokenAutoRefreshStatus[tokenId];
  if (!status)
    return "正常";
...
const refreshAllTokens = async () => {
  ...
  dialog.warning({
    title: "批量刷新Token",
    content: `确定要刷新所有支持自动刷新的Token吗？\n\n共 ${tokensToRefresh.length} 个Token需要刷新`,
    positiveText: "开始刷新",
    negativeText: "取消",
    onPositiveClick: async () => {
      try {
        ...
        for (let i = 0; i < tokensToRefresh.length; i++) {
          ...
          // 添加短暂延迟避免请求过于频繁，根据导入方式调整延迟
          const delayTime = token.importMethod === "url" ? 800 : 300; // URL请求延迟更长
        ...
        let failMessage = "";
        if (failCount > 0) {
          failMessage = failedTokens.map((ft) => `- ${ft.name}: ${ft.error}`).join("\n");
        }
        ...
        if (failCount === 0) {
          ...
          dialog.error({
            title: "批量刷新失败",
            content: `所有Token刷新失败 (共 ${failCount} 个)\n\n失败详情：\n${failMessage}`,
            positiveText: "知道了",
          });
        } else {
          // 部分失败
...
const deleteSelectedTokens = () => {
  ...
  const selectedCount = selectedTokenIds.value.size;
  dialog.warning({
    title: "批量删除账号",
    content: `确定要删除已选中的 ${selectedCount} 个账号吗？\n\n此操作将断开WebSocket连接并清除关联数据，操作不可撤销。`,
    positiveText: "确定删除",
    negativeText: "取消",
    onPositiveClick: async () => {
      let successCount = 0;
      ...
      const loadingMessage = message.loading(
        `正在删除账号 (0/${selectedCount})`,
        { duration: 0 },
      );

      for (const tokenId of selectedTokenIds.value) {
        try {
          loadingMessage.content = `正在删除账号 (${successCount + failCount + 1}/${selectedCount})`;
...
const startTaskManagement = (token) => {
  ...
  // 显示提示信息
  message.success(`正在进入 ${token.name} 的游戏功能页面`);

  // 等待一小段时间确保token状态已更新，然后跳转到游戏功能页面
  setTimeout(() => {
...
/* 深色主题下列表项样式 */
[data-theme="dark"] .token-list-item {
