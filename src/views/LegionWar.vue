    ...
    <div class="legion-war-operation">
      ...
      <div class="legion-war-operation-container">
        <div class="legion-war-operation-item">
          ...
          <div>
            <n-switch v-model:value="isOccupyOrDistribution" @update:value="handleChange"></n-switch>
          </div>
          <div>
            分布布局
          </div>
        ...
        <div class="legion-war-operation-item">
          ...
          <div>
            <n-switch v-model:value="isLegionOrIndividual" @update:value="handleChange"></n-switch>
          </div>
          <div>
            个人战况
          </div>
        ...
        <div class="legion-war-operation-item">
          <n-button style="padding:12px" type="primary" :disabled="!isEntireBattlefield" @click="getBattlefieldInfo">
            拉取数据(需进入战场后)
          </n-button>
        </div>
        <div class="legion-war-operation-item">
          <n-button style="padding:12px" type="primary" @click="sendMessageToLegion">
            发送各战队免费复活到战队频道
          </n-button>
        </div>
      </div>
...
const drawTable = (ctx, options) => {
  ...
  if (screenWidth < 768) {
    // 手机屏幕，缩小表格
    adjustScale = scale * 0.7; // 缩小30%
  } else if (screenWidth < 1024) {
    // 平板屏幕，适当缩小
    adjustScale = scale * 0.85; // 缩小15%
  }

  // 计算缩放后的实际尺寸（适配画布缩放和屏幕宽度）
  const scaledColW = columnWidth * adjustScale;
  const scaledRowH = rowHeight * adjustScale;
  const scaledFontSize = fontSize * adjustScale;
  const scaledLineWidth = 1 * adjustScale; // 边框线宽
  ...
  let currentX = x;
  const addColumnWidth = 20 * adjustScale;
  const reduceColumnWidth = 20 * adjustScale;
  // 2. 绘制表头（第1行）
  ...
  .legion-war-map {
    ...
    .map-container {
      ...
      .mapCanvas {
        ...
        height: 100%;
        display: block;
        min-width: 800px; /* Ensure minimum width for table display */
      }
    }
  }
