    ...
    <div class="card-content">
      ...
      <div class="progress-container">
        <n-progress
          ...
          :color="progressColor"
          :height="8"
          :percentage="dailyPointPercentage"
        ></n-progress>
        <div class="progress-text">{{ dailyPoint }}/110</div>
      </div>
...
const dailyBossTimesOptions = [0, 1, 2, 3, 4, 5].map((v) => ({
  label: `${v}次`,
  value: v,
}));
...
const dailyPoint = computed(() => Math.min(roleDailyPoint.value, 110));
const dailyPointPercentage = computed(() => Math.round((dailyPoint.value / 110) * 100));
const isFull = computed(() => dailyPoint.value >= 110);
