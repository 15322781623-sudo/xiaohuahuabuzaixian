...
const indigoFromItems = computed(() => getItemCount(items.value, 1019)); // 盐靖
const saltCrystalFromItems = computed(() => getItemCount(items.value, 1038)); // 盐晶
...
const resList = computed(() => {
  const allResources = [
    ...
    {
      label: "盐晶",
      value: display(saltCrystalFromItems.value as any),
      raw: getRawValue(saltCrystalFromItems.value as any),
    },
    {
      label: "晶石",
