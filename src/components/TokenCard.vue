...
const completeDailyTasks = async () => {
  ...
  try {
    ...
    try {
      ...
      addLog({
        message: `📊 ${props.token.name} 当前活跃度: ${activityPoints}/110`,
        type: "info",
      });
      ...
      if (activityPoints >= 105) {
        addLog({
          message: `✅ ${props.token.name} 活跃度已达标 ${activityPoints}/110，无需执行日常任务`,
          type: "success",
        });
        message.success(`${props.token.name} 活跃度已达标 ${activityPoints}/110，无需执行日常任务`);
      ...
      addLog({
        message: `🚀 ${props.token.name} 活跃度 ${activityPoints}，开始执行日常任务...`,
        type: "info",
      });
    } catch (error) {
      console.error("获取活跃度失败:", error);
      addLog({
        message: `⚠️ ${props.token.name} 获取活跃度失败，继续执行任务`,
        type: "warning",
      });
    }

    // 2. ✅ 加载用户配置的设置（与外部日常任务保持一致）
    // 读取全局batchSettings延迟配置（与批量日常页面保持一致）
    let batchSettings;
    try {
    ...
    const tempRunner = new DailyTaskRunner(tokenStore, {
      commandDelay: batchSettings.commandDelay || 500,
      taskDelay: batchSettings.taskDelay || 500,
    }, batchSettings);
    const settings = tempRunner.loadSettings(tokenId) || {
      arenaFormation: 1,
      bossFormation: 1,
      bossTimes: 2,
      dailyBossTimes: 1,
    ...
    addLog({
      message: `📋 加载任务配置: 竞技场=${settings.arenaEnable ? "开启" : "关闭"}, 黑市=${settings.blackMarketPurchase ? "开启" : "关闭"}, BOSS=${settings.bossTimes}次, 每日BOSS=${settings.dailyBossTimes ?? 1}次, 延迟=${batchSettings.commandDelay || 500}ms`,
      type: "info",
    });

    // ✅ 等待连接稳定后再执行任务（与外部日常任务保持一致）
    addLog({
      message: `⏳ 等待连接稳定...`,
      type: "info",
    });
    ...
    // 3. 执行每日任务补齐（使用全局batchSettings延迟配置）
    ...
    setTimeout(async () => {
      ...
      // 等待数据更新后重新加载角色信息
      setTimeout(() => {
        const updatedRoleInfo = tokenStore.selectedTokenRoleInfo;
        if (updatedRoleInfo?.dailyTask) {
          dailyTask.value.progress = Math.min(updatedRoleInfo.dailyTask.dailyPoint || 0, 110);
          dailyTask.value.complete = updatedRoleInfo.dailyTask.complete || [];
          // 更新活跃度
          if (props.token?.id) {
            tokenStore.setTokenActivity(props.token.id, updatedRoleInfo.dailyTask.dailyPoint || 0);
          }
        }
      }, 500);
    }, 1000);
