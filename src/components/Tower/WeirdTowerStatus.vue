...
const startTowerClimb = async () => {
  ...
  try {
    ...
    for (let i = 0; i < maxClimb; i++) {
      ...
      // 记录战斗前的层数（用于判断是否通关10层）
      const towerIdBeforeFight = currentTowerId.value;
      const floorBeforeFight = (towerIdBeforeFight % 10) + 1;
      const chapterBeforeFight = Math.floor(towerIdBeforeFight / 10);
      ...
      if (
        ...
        try {
          ...
          message.success(`成功领取第${chapterBeforeFight + 1}章通关奖励！`);
        } catch (rewardError) {
          // 领取奖励失败不中断爬塔流程
          console.warn(`领取通关奖励失败: ${rewardError.message}`);
        }
      }
