    ...
    <div class="club-warrank-card">
      ...
      <div ref="exportDom" class="table-content">
        ...
        <div v-if="battleRecords1 && battleRecords1.legionRankList" class="alliance-tabs-section">
          ...
          <div class="alliance-tab" :class="{ active: activeAlliance === '曦盟' }" @click="setActiveAlliance('曦盟')">
      ...
      <div v-if="playerInfo" class="player-info-content">
        ...
        <div class="action-section">
          <div style="display: flex; align-items: center; gap: 8px; flex: 1">
            ...
            <n-button size="small" style="margin-right: 8px" type="tertiary" @click="showPlayerInfoModal = false">
              关闭
            </n-button>
          </div>
      ...
      <div v-if="heroModealTemp" class="hero-modal-content">
        ...
        <div class="hero-modal-details">
          <n-descriptions bordered column="3" label-placement="left">
            ...
            <n-descriptions-item v-if="heroModealTemp.HolyBeast" label="四圣等级">
...
const handleExport1 = async () => {
  ...
  try {
    ...
    if (!exportmethod.value.includes("1") && !exportmethod.value.includes("2")) {
...
const exportToImage = async () => {
  ...
  // 获取父容器（解决高度截断问题）
  const warrankCard = exportDom.value.closest(".club-warrank-card");
  const warrankContainer = exportDom.value.closest(".club-warrank-container");
  const warrankFullContainer = exportDom.value.closest(".warrank-full-container");
  // 保存滚动位置
  const scrollTop = tableContainer ? tableContainer.scrollTop : 0;

  // 保存原始样式
  const originalExportDomStyles = {
    height: exportDom.value.style.height,
    overflow: exportDom.value.style.overflow,
    width: exportDom.value.style.width,
    maxWidth: exportDom.value.style.maxWidth,
  };

  const originalTableContainerStyles = tableContainer
    ? {
  ...
  // ✅ 保存父容器原始样式
  const originalCardStyles = warrankCard
    ? {
        height: warrankCard.style.height,
        overflow: warrankCard.style.overflow,
        minHeight: warrankCard.style.minHeight,
        maxHeight: warrankCard.style.maxHeight,
      }
    : null;

  const originalContainerStyles = warrankContainer
    ? {
        height: warrankContainer.style.height,
        overflow: warrankContainer.style.overflow,
        minHeight: warrankContainer.style.minHeight,
        maxHeight: warrankContainer.style.maxHeight,
      }
    : null;

  // ✅ 保存最外层容器原始样式
  const originalFullContainerStyles = warrankFullContainer
    ? {
        height: warrankFullContainer.style.height,
        overflow: warrankFullContainer.style.overflow,
        minHeight: warrankFullContainer.style.minHeight,
        maxHeight: warrankFullContainer.style.maxHeight,
      }
    : null;

  // ✅ 捕获当前实际内容宽度，用于固定导出宽度
  const exportWidth = exportDom.value.clientWidth || exportDom.value.offsetWidth;

  try {
    // 1. 临时调整样式，使用固定宽度而非auto
    exportDom.value.style.height = "auto";
    exportDom.value.style.overflow = "visible";
    exportDom.value.style.width = exportWidth + "px";
    exportDom.value.style.maxWidth = exportWidth + "px";

    if (tableContainer) {
      tableContainer.style.height = "auto";
      tableContainer.style.overflow = "visible";
      tableContainer.style.width = exportWidth + "px";
      tableContainer.style.maxWidth = exportWidth + "px";
    }

    // ✅ 解除父容器高度和溢出限制，避免内容被截断
    if (warrankCard) {
      warrankCard.style.height = "auto";
      warrankCard.style.overflow = "visible";
      warrankCard.style.minHeight = "auto";
      warrankCard.style.maxHeight = "none";
    }
    if (warrankContainer) {
      warrankContainer.style.height = "auto";
      warrankContainer.style.overflow = "visible";
      warrankContainer.style.minHeight = "auto";
      warrankContainer.style.maxHeight = "none";
    }
    // ✅ 解除最外层容器高度限制
    if (warrankFullContainer) {
      warrankFullContainer.style.height = "auto";
      warrankFullContainer.style.overflow = "visible";
      warrankFullContainer.style.minHeight = "auto";
      warrankFullContainer.style.maxHeight = "none";
    }

    // ✅ 2. 等待图片加载完成（手机端特别重要）
    await new Promise((resolve) => setTimeout(resolve, 1000));

    // ✅ 设置所有图片的crossOrigin属性，允许CORS加载
    const images = exportDom.value.querySelectorAll("img");
    images.forEach((img) => {
      if (!img.crossOrigin) {
    ...
    // 4. 计算实际内容尺寸（使用固定宽度，高度自适应）
    const contentWidth = exportWidth;
    const contentHeight = exportDom.value.scrollHeight;

    console.log("导出尺寸:", contentWidth, "x", contentHeight);
    ...
    const canvas = await html2canvas(exportDom.value, {
      scale, // 根据设备调整缩放比例
      useCORS: true, // 允许跨域图片
      allowTaint: false, // ✅ 禁止污染画布，与useCORS配合使用
      backgroundColor: "#ffffff", // 避免透明背景
      logging: false, // 关闭控制台日志
      height: contentHeight, // 确保捕获完整高度
      width: contentWidth, // 确保捕获完整宽度
      windowWidth: contentWidth, // 设置窗口宽度
      windowHeight: contentHeight, // 设置窗口高度
      imageTimeout: 10000, // 增加图片加载超时时间
      removeContainer: false, // 不移除临时容器
      scrollX: 0, // 确保从顶部开始截图
      scrollY: 0,
      // ✅ 使用onclone回调强制修改克隆后的DOM样式
      onclone: (clonedDoc) => {
        const fixedWidth = exportWidth + "px";

        const clonedExportDom = clonedDoc.querySelector(".table-content");
        if (clonedExportDom) {
          // ✅ 使用固定宽度，避免内容撑开
          clonedExportDom.style.width = fixedWidth;
          clonedExportDom.style.maxWidth = fixedWidth;
          clonedExportDom.style.overflow = "visible";
          clonedExportDom.style.overflowX = "hidden";
          clonedExportDom.style.overflowY = "visible";
          clonedExportDom.style.height = "auto";
          clonedExportDom.style.flex = "none";
          clonedExportDom.style.minHeight = "0";
          clonedExportDom.style.boxSizing = "border-box";
        }
        ...
        const clonedContainer = clonedDoc.querySelector(".club-warrank-container");
        if (clonedContainer) {
        ...
        // ✅ 解除最外层容器高度限制
        const clonedFullContainer = clonedDoc.querySelector(".warrank-full-container");
        if (clonedFullContainer) {
          clonedFullContainer.style.height = "auto";
          clonedFullContainer.style.overflow = "visible";
          clonedFullContainer.style.minHeight = "auto";
          clonedFullContainer.style.maxHeight = "none";
        }
        ...
        if (clonedTableContainer) {
          ...
          clonedTableContainer.style.maxWidth = fixedWidth;
          clonedTableContainer.style.overflow = "visible";
          clonedTableContainer.style.overflowX = "hidden";
          clonedTableContainer.style.overflowY = "visible";
        }

        // ✅ 强制所有table-header和table-row使用固定宽度+文字换行
        const tableRows = clonedDoc.querySelectorAll(".table-header, .table-row");
        tableRows.forEach((row) => {
          row.style.width = "100%";
          row.style.maxWidth = fixedWidth;
        ...
        // ✅ 约束hero-avatars不超出行宽度
        const heroSections = clonedDoc.querySelectorAll(".hero-avatars");
        ...
        // ✅ 约束hero-card不超出可用空间
        const heroCards = clonedDoc.querySelectorAll(".hero-card");
        heroCards.forEach((card) => {
          card.style.minWidth = "0";
          card.style.maxWidth = card.style.maxWidth || "95px";
          card.style.overflow = "hidden";
          card.style.boxSizing = "border-box";
        });

        // ✅ 约束table-cell文字不溢出
        const tableCells = clonedDoc.querySelectorAll(".table-cell");
        ...
        // ✅ 约束名称单元格不超长
        const nameCells = clonedDoc.querySelectorAll(".table-cell.name");
    ...
    // 6. Canvas转图片链接并下载
    // 生成安全的文件名，避免特殊字符
    const safeDate = queryDate.value.replace(/\//g, "-");
    const filename = `${safeDate}-盐场匹配信息.png`;
    ...
    try {
      ...
      // 再次尝试下载
      await downloadCanvasAsImage(resizedCanvas, filename);
      // 移除成功提示，实现自动下载
    }
  } catch (err) {
    ...
    // 恢复原始样式
    if (exportDom.value) {
      exportDom.value.style.height = originalExportDomStyles.height || "";
      exportDom.value.style.overflow = originalExportDomStyles.overflow || "";
      exportDom.value.style.width = originalExportDomStyles.width || "";
      exportDom.value.style.maxWidth = originalExportDomStyles.maxWidth || "";
    }

    if (tableContainer && originalTableContainerStyles) {
    ...
    // ✅ 恢复父容器样式
    if (warrankCard && originalCardStyles) {
      warrankCard.style.height = originalCardStyles.height || "";
      warrankCard.style.overflow = originalCardStyles.overflow || "";
      warrankCard.style.minHeight = originalCardStyles.minHeight || "";
      warrankCard.style.maxHeight = originalCardStyles.maxHeight || "";
    }
    if (warrankContainer && originalContainerStyles) {
      warrankContainer.style.height = originalContainerStyles.height || "";
      warrankContainer.style.overflow = originalContainerStyles.overflow || "";
      warrankContainer.style.minHeight = originalContainerStyles.minHeight || "";
      warrankContainer.style.maxHeight = originalContainerStyles.maxHeight || "";
    }
    // ✅ 恢复最外层容器样式
    if (warrankFullContainer && originalFullContainerStyles) {
      warrankFullContainer.style.height = originalFullContainerStyles.height || "";
      warrankFullContainer.style.overflow = originalFullContainerStyles.overflow || "";
      warrankFullContainer.style.minHeight = originalFullContainerStyles.minHeight || "";
      warrankFullContainer.style.maxHeight = originalFullContainerStyles.maxHeight || "";
    }
  }
};
...
@media (max-width: 768px) {
  ...
  // 编辑模式适配
  .table-cell.rank .edit-rank {
    :deep(.n-input-number) {
      width: 55px !important;
    }
  }

  .table-cell.alliance .edit-alliance {
