        ...
        <div v-else class="saved-lineups-modal-content">
          <div class="team-tabs">
            <div class="team-tabs-left">
              ...
              >
                槽位{{ teamId }}
                <span class="tab-count">({{ getLineupsByTeamId(teamId).length }})</span>
              </div>
            </div>
...
const confirmHeroAction = async () => {
  ...
  if (exchangeMode.value === "add") {
    ...
    originalArtifactId = exchangeHero.value.artifactId;
    originalPosition = exchangeHero.value.position;
  ...
  try {
    const fetchLatestData = async () => {
      ...
      const artifactBooks = roleInfo?.role?.artifactBooks || roleInfo?.artifactBooks || {};
      const pearlMap = roleInfo?.role?.pearlMap || roleInfo?.pearlMap || {};
      roleHeroesData.value = heroes;
      artifactBooks.value = artifactBooks;
      pearlMap.value = pearlMap;
      const team
    ...
    const { heroes, teamInfo, artifactBooks, pearlMap } = await fetchLatestData();
    ...
    const targetHeroId = exchangeTargetHeroId.value;

    const targetHeroData = roleHeroesData.value[String(targetHeroId)];
    const targetAttachmentUid = targetHeroData?.attachmentUid;
    ...
    if (exchangeMode.value === "exchange" && exchangeHero.value) {
      const originalHeroId = exchangeHero.value.heroId;
      const position = exchangeHero.value.position;
      const currentHeroAtPos = currentHeroes.find((h) => h.position === position);

      if (currentHeroAtPos && currentHeroAtPos.heroId === originalHeroId) {
        try {
          await tokenStore.sendMessageWithPromise(
            tokenId,
            "hero_gobackbattle",
      ...
      // 装备继承逻辑
      if (originalArtifactId && originalArtifactId !== -1) {
        // 检查当前装备持有者
        ...
        const currentHolderId = artifactToHero[originalArtifactId];

        if (currentHolderId && currentHolderId !== targetHeroId) {
          // 从原持有者卸载装备
          try {
            await tokenStore.sendMessageWithPromise(
              tokenId,
        ...
        try {
          await tokenStore.sendMessageWithPromise(tokenId, "artifact_load", {
            heroId: targetHeroId,
            itemId: originalArtifactId,
            pearlId: 0, // 暂时设为0，后续可以从原装备获取
          });
        } catch (err) {}
        await delay(COMMAND_DELAY);
      }
