...
async function startSingleBattle(heroId) {
  ...
  try {
    ...
    if (response) {
      ...
      const dungeonId = response.dungeonId || response.stageId || 0;
      const star = response.star || 0;

      if (isWin) {
        console.log(`${heroName} 战斗胜利!${star ? `（${star}星）` : ""}`);
      } else {
      ...
      if (dungeonId) {
        try {
          ...
          console.warn(`${heroName} 梦境奖励领取失败:`, rewardErr.message);
        }
      }
