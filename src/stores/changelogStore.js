  ...
  const changelogs = ref([
    {
      ...
      type: "patch",
      title: "盐场刨地消息接收崩溃修复（binaryType 未设置致 Blob 卡死解析）+ 刨地无可用相邻格运行时诊断（区分等待落位/地图缺格、输出坐标与地图状态）+ 游戏登录页增强「升星助手」脚本整体升级",
      fixes: [
        "修复盐场刨地「消息处理失败: Cannot read properties of undefined (reading '_raw')」崩溃：盐场 WebSocket 连接未设置 binaryType，浏览器默认以 Blob 形式接收二进制帧，而消息入口只处理了字符串与 ArrayBuffer 两种类型——服务器心跳/进场/推送其实都已到达，但每次一进消息处理就因 packet 为 undefined 抛错，请求响应永远匹配不上、任务卡死；现连接建立后立即设置 binaryType='arraybuffer'，并将消息处理拆分为可复用的解析（ArrayBuffer/Blob 双路径）与统一处理，补充 Blob 异步兜底与空包防御，对齐项目内军团战地图模块的成熟处理方式",
        "刨地「无可用相邻格」原地等待新增运行时诊断：区分两种本质不同的等待——「等待落位」（角色坐标无效/未下发出生格，打印当前坐标与地图格子数）与「地图缺格」（有坐标但 buildingData 无相邻格）分别提示；每次重新拉取战场全量地图时输出角色坐标、地图已收到的格子数量与样例、期望的 6 个相邻格 key、以及最近收到的 8 条服务器消息命令，一键定位是 buildingData 未合并、全量地图请求未响应，还是坐标/邻居方向规则不匹配",
        "游戏增强「升星助手」脚本整体升级：登录页游戏增强列表中的升星助手由旧版 fish_hero_star.js（CC 助手提取版，仅含基础升星/图鉴/鱼灵逻辑）替换为游戏同款新版 star_upgrade.js（武将/图鉴/鱼灵完整自动升星逻辑），旧脚本已删除；增强列表对应项同步更新（fishHeroStar → starUpgrade），并移除升星助手对面板增强器（panel_enhancer）的依赖注入，新版脚本独立完整运行，升星操作不再受面板增强器状态影响",
      ],
    },
    {
      version: "v2.51.2",
      date: new Date().toISOString().split('T')[0],
      type: "minor",
      title: "新增营地挑战批量任务（三策略选人/排除对手/畏首畏尾/对手阵容缓存/自动领奖）+ 盐场刨地协议级修复（sid2 用错/心跳错误/响应解析失败/补全地图拉取）+ 布阵默认跟随账号当前出战阵容",
      fixes: [
        "新增「营地挑战」批量任务：批量日常页「俱乐部」分页新增「营地挑战」按钮，点击弹出挑战设置弹窗，勾选账号即可手动批量执行；「新增定时任务」勾选任务列表中的「营地挑战」即可按计划自动执行，定时任务内可单独保存挑战设置（任务级优先，未配置时按全局营地挑战设置执行）",
        "挑战流程：读取账号营地信息（club_getinfo）获取今日对阵的防守者列表（30 节点：上中下三区 × 普通/统帅骁将/困难/炼狱难度），按 nodeId/targetId/challengeCnt/failCnt 携带出战阵容对 club_attack 发起挑战，实时同步服务端今日已攻击/已击败次数与各节点防守进度（胜利后按规则重选目标、失败保持同一对手直到连败上限）；全部打完后自动检查并领取营地任务各档奖励（club_taskclaim），已领取/未达标的分档跳过",
        "挑战策略三选一：清空区域优先——锁定剩余可击败数最少的一路（优先下区再中区最后上区）集中攻克，最大化清空区域拿首通奖励，同分随机避免死磕同一人；挑战奖励优先——结合目标筛选只打有奖励的统帅/骁将、困难/炼狱等防守者；仅空投——不碰真人，只挑战虚拟对手稳稳拿击败数",
        "目标筛选（仅挑战奖励优先时生效）：无限制 / 仅统帅骁将 / 仅困难炼狱 / 两者都打；筛选后无可用对手时自动忽略这些条件重新选择，保证不空跑",
        "排除对手：勾选即不打指定成型阵容（吴国/姜维/毒爆/吕赵/三蜀/典韦/司马懿/关羽/吕布/控制毒/控制吴，与图鉴阵容识别同规则）；判定要求命中武将全部 30 星满级才生效，防止对手刻意降级绕过误伤",
        "畏首畏尾（默认开启，可一键总关闭）：挑战前先查看防守者出战阵容，对手战力 / 总红淬炼达到自身设定倍率（默认 1.0 倍，0.8~2.0 可调）即跳过换下一个目标，不打没把握的仗；关闭后不因对手过强而退缩",
        "屡战屡败保护：同一对手连续失败达到上限（默认 3 次，1~10 可调）自动放弃该节点并切换对手，本次执行不再挑战该节点，避免死磕浪费次数",
        "行为管理：剩余攻击次数已无失败余地（剩几次就要再击败几人）时的处理——默认转空投挑战虚拟对手保底凑满击败数，也可选直接暂停真人挑战去领奖，或把剩余次数全部打完由胜负说话",
        "出战阵容：默认切换为账号设置中的「竞技场阵容」出战（切阵前记录当前出战阵容，挑战结束后自动还原），也可选择不切换直接用当前阵容；自动识别营地通行证——有效时攻击/击败上限为 15 次/5 人，无效时 10 次/3 人",
        "对手阵容缓存（localStorage）：查看过的防守者阵容按「对手俱乐部或自身俱乐部 ID + 角色 ID + 当天日期」为键缓存，有效期一天且同俱乐部账号共享——同一防守者当天不再重复请求接口，畏首畏尾/排除判定直接读缓存秒过；写入时顺带清理两天前的过期缓存防止无限膨胀",
        "盐场布阵默认跟随账号当前出战阵容：不再固定使用某套预设队，进场布阵前先从主服读取 presetteam_getinfo，取「当前出战阵容」（useTeamId）对应的武将明细按 battleTeam（槽位→武将ID）协议下发布阵，所见即所用——游戏里当前用哪队，盐场就带哪队；账号设置/任务模板的「盐场蟠桃阵容」下拉新增「跟随当前出战阵容」选项并设为默认，仍可显式指定 1-6 号预设队覆盖；「盐场蟠桃阵容」批量切换按钮同步支持 0=跟随当前（无需切换）",
        "盐场布阵参数修正为 battleTeam（服务端 SaltFieldRunner 同款协议）：此前发送 teamId 指定阵容编号，与盐场服务器实际接受的 battleTeam（槽位→武将 heroId）不一致，布阵可能不生效；现从账号预设队数据（teamInfo）构造 {槽位: heroId} 发送 war_teamsetbattleteam / war_setbattleteam；目标预设队为空或主服预设读取失败时自动回退吕布单将（{'0':107}）保证能出城刨地，日志会注明实际布阵来源与人数",
        "修复盐场专用 WebSocket 连接 URL 中 sid2 参数用错：此前误把战场 ID（battlefieldId）填进 sid2，legion_getbattlefield 返回的服务器 ID（sid）被闲置，连接无法正常进入战场；现改为使用服务器 ID，与已工作的军团战地图模块及文档服务端实现一致",
        "修复盐场心跳协议错误：此前按主服习惯发送 cmd=heart_beat，盐场服务器不识别该命令；现按盐场协议直接发送 cmd=war_ping + seq=0 + body={ battlefieldId }，并补齐首拍定时器清理，避免长时任务被服务端判定离线断开",
        "修复进场响应解析失败导致必然报「未获取到 roleCodeId」的问题：盐场响应经 BON 解码后 roleCodeId/battlefield 位于报文业务数据顶层（rawData/decodedBody），原代码从 enterResult.body 取永远取不到；新增 pickField 统一兼容多层结构提取",
        "修复战场增量推送从未生效的问题：推送包 body 为二进制数据，原代码检查 packet.body.battlefield 永远取不到内容，角色位置/状态从不更新，任务会一直卡在等待或原地；现改为从解码后的 rawData/decodedBody/body 中提取 battlefield 并合并进本地快照",
        "进场后主动拉取一次战场全量地图（war_getbattlefieldinfo）：buildingData 全量地图仅由该命令下发、服务器只推送变化部分，不主动拉取则永远提示「无可用相邻格」无法刨地；刨地循环连续多次无可用相邻格时自动重新拉取",
        "连接建立后等待 3 秒再进场（对齐服务端 SaltFieldRunner），避免刚连上立即进场被拒；布阵响应携带的 battlefield 也一并合并进快照",
        "修复 callWithRetry 重试形同虚设的问题：catch 分支无条件 throw，for 循环的 retries 参数从未真正生效；现按错误类型区分——明确不可重试的错误直接抛出，其余按配置退避后重试",
        "清理未使用的旧版盐场连接死代码（createSaltFieldWS 明文 JSON 协议版与 pickRandomNeighbor），避免与 BON 协议客户端混淆",
      ],
    },
    {
      version: "v2.51.1",
      date: new Date().toISOString().split('T')[0],
      type: "patch",
      title: "修复应用宝扫码登录固定 7 天掉线 + 批量推图多项优化（定时刷新/定时互斥/nextTime复用/心跳去重/200020不计负）+ 协议服务CORS/远程停机 + APK构建体积与混淆修复",
      fixes: [
        "修复应用宝扫码登录每隔 7 天固定掉线的问题：协议服务（yyb-go）的自动续期触发条件原为「距上次刷新满 7 天」，而应用宝 refresh_token 有效期也约为 7 天且属于滑动续期（必须在有效期内使用换新才能续命）——等到第 7 天触发刷新时票据恰好过期，导致账号周期性掉线只能重新扫码",
        "续期策略调整为每 24 小时滑动刷新一次：refresh_token 在 7 天有效期内持续换新续命；即使某次刷新失败（断网/应用未运行），有效期内仍约 6 次自动重试机会，正常使用下登录态永久保活；yyb_go2 补齐缺失的自动续期启动与续期参数，与主服务策略一致",
        "提示：已掉线（票据已过期）的账号无法自动恢复，需重新扫码一次；升级后新版协议服务会每天自动续期，不再出现 7 天固定掉线（连续 7 天以上完全不打开应用属协议限制，仍会过期）",
        "协议服务补齐 Web 端直连能力：新增 CORS 跨域放行（浏览器可直连本机服务，含 OPTIONS 预检）与 /shutdown 远程停机接口（Web/APK 端无法杀外部进程时让服务自行优雅退出，先回响应再停机）",
        "修复定时刷新页面把批量推图账号一起清理、无法恢复的问题：刷新安全检查（isSafeToRefreshPage）此前只看 isRunning/runningTokens，而推图走 window._pt 独立状态导致误判为安全直接 reload；现新增推图运行检查，有账号推图中一律推迟刷新，推图全部结束后由调度器自动执行之前推迟的刷新",
        "新增定时任务与推图互斥（定时优先）：定时任务执行前自动暂停推图（window._pausePushRequested 标记，推图在每轮战斗前/每次发送前/倒计时中检测并暂停等待），等推图停稳后再执行定时，定时完成后推图自动重连继续；健康检查各强制释放分支同步清除暂停标记，避免推图永久卡在暂停态；账号卡片推图状态新增「等待定时完成」暂停显示",
        "推图性能优化：复用上轮 fight_level 返回的 nextTime 作为下一轮战斗等待时间，跳过一次 fight_calcleveltime 请求（稳定推图每轮 3 请求降为 2 请求）；复用后立即消费清空，重连/最终失败后清空缓存重新 calc，同样受 30 分钟上限保护",
        "推图心跳断开日志去重：sendHeartbeat 改为返回布尔值，断开时不再打误导性「下次心跳重连」日志，由倒计时检查统一输出一句并直接进指数退避重连，消除每 10 秒成对刷屏",
        "推图 200020 最终失败不再记负场：fight_level 4 次重试仍失败（多为服务端战斗未结算）时跳过本轮且不计胜负，避免胜率失真，同时清空缓存的 nextTime 下一轮重新 calc",
        "修复 APK 构建体积异常膨胀（49MB 涨到 72MB）：资源同步备份目录残留在 assets 内被 Gradle 一并打进 APK；现备份改放 assets 之外（同卷 rename 挪走，不触发受限环境批量删除保护），并清理历史残留的 public.bak-* 目录",
        "修复前端混淆偶发跳过文件（EPERM）：vite 拷贝到 dist 的副本继承源文件只读属性，writeFileSync 直接报错跳过；现写入前统一清除只读属性，内联脚本混淆同步处理",
      ],
    },
    {
      version: "v2.51.0",
      date: new Date().toISOString().split('T')[0],
      type: "minor",
      title: "新增营地战报模块：当日营地攻防战报展示 + 敌我三路成员明细（敌方仅显示真实对打对手、头像多路径兜底） + 布局紧凑化与电脑/手机自适应 + 账号卡片灯神挑战状态（游戏原图标 + 本地缓存显示）",
      fixes: [
        "账号卡片新增灯神挑战状态标签：卡片实时显示「通关 X/5」（已通关势力数/总势力数），标签图标使用游戏内灯神金像原图（替代 emoji），未获取到时显示「未获取」，点击可查看魏/蜀/吴/群雄/深海各势力已通关层数明细，数值为已通关层数（魏蜀吴群 17 层、深海 10 层为全部通关），全部通关时标签整体绿色高亮",
        "灯神挑战状态本地缓存显示：标签数据按账号缓存到本地（genie_status_账号ID），账号未刷新或连接断开、在线数据缺失时自动回退展示最近一次成功刷新的缓存进度（标签与明细弹窗均生效），不再直接显示「未获取」；首次成功刷新账号数据后自动写入缓存，有实时数据时仍以实时数据为准",
        "新增「营地战报」模块入口：游戏功能页新增「营地战报」分区，数据来自 club_getinfo（俱乐部营地信息），自动展示当日营地挑战「攻防战报」，支持手动刷新与空数据/加载状态提示",
        "战报头部信息：展示营地挑战战报标题（南瓜战场/大荒），攻防总览卡片含「总攻打塔数 / 防守失败数」统计，并分「我方胜率 / 我方防守胜率」两侧展示总攻打·成功·失败与总防守·成功·三路攻的百分比与数量条形图",
        "小队周榜 Top3：按排名展示奖次数最多的小队成员与获奖次数，榜首高亮标识",
        "敌方展示修正：敌方营地（VS 区）与敌方成员明细只展示真正开打、有俱乐部总篝火数值（dayScore>0）的敌方俱乐部——未参战（dayScore=0）的占位俱乐部不再混入人数统计与成员明细；无有效对打对手时 VS 区显示「暂无对打」空态，不再出现灰星/0 假数据",
        "我方营地头像兜底修复：我方营地头像此前经常取不到而显示占位图，现支持多字段路径提取（logo/legionLogo/icon/avatar/club.*），俱乐部无 logo 时自动回退为营地首位成员（先锋）头像，不再空白",
        "我方/敌方成员明细：按上中下三路分组展示攻防双方全部成员数据，每条成员记录含排名、头像、名字与状态标签（先锋/首位、我方失守等）、击杀/被攻击/被击败/被挑战四项攻防统计，失守成员红色高亮标记",
        "战报一键导出：支持将整份营地战报（头部统计 + 敌我全部成员明细）导出为图片，方便分享",
        "成员布局紧凑化：成员记录改为单行排版（排名｜头像｜名字｜四维数据同排显示），统计数字大字号 + 小字单位上下排列，头像与卡片间距同步压缩，一屏可容纳更多成员，视觉对齐咸鱼监控战报风格",
        "电脑/手机自适应：成员区按屏幕宽度自动排布为 3/2/1 列，字号、间距随视口 clamp 缩放，手机窄屏自动堆叠单列，完整展示所有字段",
        "修复电脑端营地战报无法滚动查看的问题：此前「取消固定高度」的样式只写在手机端媒体查询内，电脑端容器被限高并 overflow:hidden，内容超出即被裁剪；现改为全局取消固定高度与裁剪，战报内容自然撑开、整页纵向滚动，电脑端与手机端均可完整查看底部内容",
      ],
    },
    {
      version: "v2.50.9",
      date: new Date().toISOString().split('T')[0],
      type: "minor",
      title: "灯神挑战阵容统一走账号设置：移除各处「使用阵容」手动选择 + 进度层号显示修正 + 操作说明提示",
      fixes: [
        "灯神挑战阵容统一使用账号设置：灯神挑战弹窗与批量任务卡中的「使用阵容」下拉已移除，执行时统一读取账号设置/任务模板中的「灯神预设阵容」（1-6，所有势力共用），避免多入口各自配置导致用错阵容",
        "取消「自动（按势力选同阵营队）」模式：阵容来源仅剩账号设置的灯神预设阵容；旧数据兼容，旧值 0/非法值自动回退阵容 1",
        "挑战前先刷新阵容数据：执行灯神挑战前先调用游戏功能的阵容刷新，重新拉取账号预设阵容后再获取进度，确保读到最新配置",
        "进度层号语义修正：role.genie 记录「已通关层-1」（12=已通关游戏内第13层），服务器按自身进度自动打下一层；日志改为「当前进度通关在第X层，挑战第X+1层」，战斗成功/失败/重试层号均与游戏内实际层数一一对应，消除差一层的误报",
        "灯神挑战入口新增操作说明：先在游戏内设置好灯神阵容并在灯神挑战界面完成预设阵容调整后挑战一次，后续通过账号设置选择预设阵容、再勾选对应势力即可挑战",
      ],
    },
    {
      version: "v2.50.8",
      date: new Date().toISOString().split('T')[0],
      type: "minor",
      title: "灯神挑战批量任务完善：从真实进度层开始挑战 + 每日次数读取与 10 次上限 + 胜负判定修复 + 日志精简",
      fixes: [
        "从真实进度层继续挑战：不再固定从第 1 层开打，挑战前读取账号灯神当前进度（已通关层数），从下一层开始逐层挑战，胜利后自动推进下一层，直到失败或当日次数用尽",
        "每日挑战次数控制（默认上限 10 次，可配置）：开打前自动读取服务器今日已挑战次数（智能识别 genie:battle 等统计键，识别失败时兜底扫描今日 genie 相关统计），按剩余次数发起挑战；次数已用尽时明确提示「今日已挑战 X/10 次」并立即停止，不再盲目打到失败",
        "挑战结果判定修复：战斗响应按服务器实际返回层级正确解析，成功时输出「我方剩余 / 敌方剩余」；失败即停止该势力，血量字段缺失时通过刷新角色进度做兜底判定，杜绝无法判定继续空耗",
        "战斗处理中自动重试：服务器返回处理中(200020)时等待 1 秒自动重试发起，避免偶发繁忙直接误判失败",
        "支持多势力/预设阵容/自定义每日上限：批量日常弹窗可选择挑战势力（魏/蜀/吴/群）、预设阵容与每日总次数上限，定时任务配置同步支持",
        "日志精简：移除开打前「灯神统计键」探测信息输出（该行仅用于开发校准），保留进度层、胜负与次数等关键日志",
      ],
    },
    {
      version: "v2.50.7",
