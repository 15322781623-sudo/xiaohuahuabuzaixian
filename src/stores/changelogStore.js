  ...
  const changelogs = ref([
    {
      ...
      type: "minor",
      title: "连接失败自动刷新Token重连优化 + 游戏增强合并自动重连/面板全选恢复默认/UI文字换行 + 废弃文件清理",
      fixes: [
        "修复连接失败重连成功率低的问题：WebSocket 连接首次失败时自动先尝试刷新 Token 再重连——很多「连接超时」实际是 Token 过期导致，新 Token 可显著提高重连成功率；复用 tokenStore.attemptTokenRefresh 内置保护（refreshingTokenIds 防重复、10-20 秒冷却、最多 3 次内部重试），全程仅刷新一次不会反复浪费接口配额",
        "刷新成功智能直连：Token 刷新成功后跳过 30 秒阶梯退避，等待 2 秒让新 Token 生效后立即进入下一轮重连；刷新失败则按原计划阶梯退避（30秒/1分钟/3分钟）重试，不影响原有重试逻辑",
        "重试等待可随时停止：阶梯退避改为每秒分段等待，用户点击停止后立即中断等待抛错退出，不再长时间无响应",
        "任务停止即时响应：ensureConnection 循环开头新增 shouldStop 检查，用户中途停止时立即退出，避免无效的长时间连接等待",
        "游戏增强面板新增「全选 / 取消 / 恢复默认」按钮：一键开启全部增强、一键关闭全部增强、一键恢复官方推荐默认配置（跳过弹窗、广告跳过、性能优化、战斗飘字、自动重连等默认启用项），登录页游戏增强区与游戏窗口悬浮工具栏两处入口同步支持",
        "十殿加速默认关闭：为避免新用户首次使用时游戏节奏被意外加速，十殿加速不再默认启用，需要时在游戏增强面板手动开启；已保存的个性化配置不受影响，点击「恢复默认」后同样按新默认值生效",
        "游戏增强新增「宠物合成」：游戏内宠物合成增强脚本，位于升级分组，可在游戏增强面板手动开启；默认关闭，启用后自动注入游戏并支持面板增强器（拖动转移 + 手机/电脑宽高自适应）",
        "游戏增强「自动重连」合并「断线重连」：统一为一个开关一个脚本（引擎级自动重连：断线立即重连 0 秒延迟、reset+connect 原连接参数恢复、心跳保活、软重连 3 次仍失败才兜底刷新页面）；「断线重连」独立开关与脚本已移除，已保存的旧配置自动迁移（旧开启状态并入自动重连），旧缓存中的残留代码自动过滤，不再重复 Hook 重复刷新；自动重连描述文案同步精简",
        "游戏增强 UI 文字换行优化：增强项名称与描述支持自动换行完整展示（长描述不再被截断为省略号），开关与多行文字顶部对齐，登录页侧栏增强区与游戏窗口悬浮工具栏两处同步优化",
        "清理废弃功能与代码体积优化：移除「踢下线」「十殿抽奖」「桃花谷」「星驰免登录」4 个废弃增强脚本（约 2MB），游戏增强面板与内置缓存同步更新，不再注入无效代码；保留本地资源解析（local_res_dds.js）",
      ],
    },
    {
      version: "v2.50.6",
      ...
      fixes: [
        ...
        "修复营地空投挑战次数次日不重置的问题：读取已挑战次数时错误地取 attackMap 中键值最大的历史记录（昨天）当作今日次数，导致今天直接提示「次数已用完」跳过挑战；现改为只统计今天日期键（yymmdd/yyyymmdd 兼容）的已挑战次数，今天无记录即正常发起挑战，由服务器返回 2600040 做次数上限兜底",
      ],
    },
    {
      version: "v2.50.5",
